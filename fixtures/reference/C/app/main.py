"""URL shortener service (reference implementation, stage C: optional expiry). EXPIRED_STATUS is the
requirement-defined response for expired links (404 in the initial clarification, 410 after revision)."""
...
def create_app(db_path: str, base_url: str = "http://localhost:8000", now=None, code_generator=None) -> FastAPI:
    ...
    def ensure_schema() -> None:
        ...
        try:
            ...
            if "expires_at" not in cols:
                # Additive, nullable migration: existing rows keep NULL and never expire.
    ...
    def insert_link(conn: sqlite3.Connection, code: str, url: str, created_at: str, expires_at: str | None) -> bool:
        try:
            ...
            if conn.execute("SELECT 1 FROM links WHERE code=?", (code,)).fetchone():
                ...
                return False
            conn.execute(
                "INSERT INTO links(code, target_url, created_at, click_count, expires_at) VALUES(?,?,?,0,?)",
    ...
    async def create_link(request: Request):
        ...
        if expires_raw is not None:
            if not isinstance(expires_raw, str):
                return error(422, "expires_at must be an ISO-8601 timestamp with timezone")
            exp = _parse_ts(expires_raw)
            if exp is None:
                return error(422, "expires_at must be an ISO-8601 timestamp with timezone")
            if exp <= current:
                return error(422, "expires_at must be in the future")
            expires_at = _iso(exp)
        ...
        try:
            if alias is not None:
                try:
                    ok = insert_link(conn, alias, url, created_at, expires_at)
                except sqlite3.Error:
                    return error(503, "database unavailable")
                if not ok:
                ...
                for _ in range(CODE_ATTEMPTS):
                    ...
                    try:
                        if insert_link(conn, candidate, url, created_at, expires_at):
                            code = candidate
                            break
                    except sqlite3.Error:
                        return error(503, "database unavailable")
            ...
            return JSONResponse(
                {"code": code, "short_url": f"{base}/r/{code}", "url": url, "created_at": created_at, "expires_at": expires_at},
                status_code=201,
            )
        finally:
            conn.close()
    ...
    async def redirect(code: str, request: Request):
        ...
        try:
            if request.method == "HEAD":
                ...
                if is_expired(row[1]):
                    return Response(status_code=EXPIRED_STATUS)
            ...
            try:
                ...
                if is_expired(row[1]):
                    conn.execute("ROLLBACK")
                    return error(EXPIRED_STATUS, "link expired")
    ...
    async def stats(code: str):
        try:
            ...
            return error(503, "database unavailable")
        try:
            row = conn.execute("SELECT code, target_url, created_at, click_count, expires_at FROM links WHERE code=?", (code,)).fetchone()
        except sqlite3.Error:
            return error(503, "database unavailable")
        finally:
            conn.close()
        ...
        return {
            ...
            "click_count": row[3],
            "expires_at": row[4],
            "status": "expired" if is_expired(row[4]) else "active",
        }
