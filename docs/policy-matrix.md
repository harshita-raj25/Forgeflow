...
| Stale lease holder cannot publish after ownership moves to another scheduler | Yes | `tests/test_review_fixes_3.py::test_stale_lease_holder_cannot_publish_after_takeover`, `::test_concurrent_resume_after_real_takeover_full_pipeline` | `_promotion_blocked` checks `lease_owner == self.owner` under `_promote_lock` before requirement/graph/status checks |
