...
Run: `run_8zs4g7c8d23s` (fixture) — `runs/run_8zs4g7c8d23s/export/`
...
Normal run: `run_3349z476wue9` (fixture) — baseline is scenario A's own release
(`runs/run_8zs4g7c8d23s/export/release/candidate`), auto-resolved by
`Coordinator.latest_release_dir("A_greenfield")`.

Evidence: `runs/run_3349z476wue9/export/decisions.json` impact map cites real paths/symbols from the
...
`run_rdmr5uxxhe6j` (fixture, `--inject-fault`). `events.jsonl` shows, in order: a `fault_injected` event
...
defect. A normal (non-injected) run (`run_3349z476wue9`) is kept as the product evidence.

## Scenario C — ambiguous requirement + upstream replanning

Run: `run_t5ktyzfxjjhh` (fixture) — baseline is scenario B's release.
